...
const team = [
  {
    ...
    commitNames: ["karenheckel", "Karen Heckel"],
  ...
  },
  {
    ...
    commitNames: ["Parul S Sadasivuni", "Parul", "parul.sadasivuni"],
  ...
  },
  {
    ...
    commitNames: ["Brianna-Flo", "Brianna Flores"],
  ...
  },
  {
    ...
    commitNames: ["anovruzov", "Ali Novruzov"],
      ...
      try {
        const results = await Promise.all(
          team.map(async (member) => {
            ...
            let totalCommits = 0;
            for (const username of member.commitNames) {
              let page = 1;
              let commitsPage;
              do {
                const res = await fetch(
                  `https://gitlab.com/api/v4/projects/${projectId}/repository/commits?per_page=${perPage}&page=${page}&author=${encodeURIComponent(username)}`
                );
